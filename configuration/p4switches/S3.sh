...
# host 
tc qdisc add dev host handle 1: root htb default 14 direct_qlen 1000000
tc class add dev host parent 1: classid 1:1 htb rate 12Mbit ceil 12Mbit burst 15k cburst 15k
    ...
    # gold
    tc filter add dev host parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 128 0xff flowid 1:11
    
    # silver
    tc filter add dev host parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 64 0xff flowid 1:12
    
    # bronze
    tc filter add dev host parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 32 0xff flowid 1:13

    ## leaves

    tc qdisc add dev host parent 1:11 handle 10: sfq limit 16256 quantum 1500
    tc qdisc add dev host parent 1:12 handle 20: sfq limit 16256 quantum 1500
    tc qdisc add dev host parent 1:13 handle 30: sfq limit 16256 quantum 1500
    tc qdisc add dev host parent 1:14 handle 40: sfq limit 16256 quantum 1500
...
tc class add dev port_R2 parent 1: classid 1:1 htb rate 6Mbit ceil 6Mbit burst 15k cburst 15k
    ...
    # gold
    tc filter add dev port_R2 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 128 0xff flowid 1:11
    
    # silver
    tc filter add dev port_R2 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 64 0xff flowid 1:12
    
    # bronze
    tc filter add dev port_R2 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 32 0xff flowid 1:13
...
tc class add dev port_R3 parent 1: classid 1:1 htb rate 4Mbit ceil 4Mbit burst 15k cburst 15k
    ...
    # gold
    tc filter add dev port_R3 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 128 0xff flowid 1:11
    
    # silver
    tc filter add dev port_R3 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 64 0xff flowid 1:12
    
    # bronze
    tc filter add dev port_R3 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 32 0xff flowid 1:13
...
tc class add dev port_S4 parent 1: classid 1:1 htb rate 6Mbit ceil 6Mbit burst 15k cburst 15k
    ...
    # gold
    tc filter add dev port_S4 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 128 0xff flowid 1:11
    
    # silver
    tc filter add dev port_S4 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 64 0xff flowid 1:12
    
    # bronze
    tc filter add dev port_S4 parent 1: protocol all prio 1 u32 match ip protocol 17 0xff match ip tos 32 0xff flowid 1:13
