...
sudo ip netns exec router tc class add dev router-eth3 parent 1:1 classid 1:10 htb rate 1Mbit ceil 10Mbit prio 1 burst 15k cburst 15K
sudo ip netns exec router tc filter add dev router-eth3 parent 1: protocol ip prio 1 u32 match ip protocol 1 0xff match ip dsfield 128 0xff flowid 1:10


# medium  priority silver TOS 64
sudo ip netns exec router tc class add dev router-eth3 parent 1:1 classid 1:20 htb rate 4Mbit ceil 10Mbit prio 2 burst 15k cburst 15K
sudo ip netns exec router tc filter add dev router-eth3 parent 1: protocol ip prio 1 u32 match ip protocol 1 0xff match ip dsfield 64 0xff flowid 1:20


#low  priority silver  TOS 32
sudo ip netns exec router tc class add dev router-eth3 parent 1:1 classid 1:30 htb rate 12Mbit ceil 12Mbit prio 3 burst 15k cburst 15K
sudo ip netns exec router tc filter add dev router-eth3 parent 1: protocol ip prio 1 u32 match ip protocol 1 0xff match ip dsfield 32  0xff flowid 1:30
