...
# S4
## add back the mtu field? 
tc qdisc add dev port_S4 handle 1: root htb default 14
tc class add dev port_S4 parent 1: classid 1:1 htb rate 6Mbit ceil 6Mbit burst 15k

    # gold
    tc class add dev port_S4 parent 1:1 classid 1:11 htb rate 1Mbit ceil 6Mbit burst 15k cburst 15K prio 1 
    tc filter add dev port_S4 parent 1: protocol ip prio 1 u32 match ip protocol 17 0xff match ip tos 128 0xff flowid 1:11

    #silver
    tc class add dev port_S4 parent 1:1 classid 1:12 htb rate 4Mbit ceil 6Mbit burst 15k cburst 15K prio 2
    tc filter add dev port_S4 parent 1: protocol ip prio 1 u32 match ip protocol 17 0xff match ip tos 64 0xff flowid 1:12

    # bronze
    tc class add dev port_S4 parent 1:1 classid 1:13 htb rate 6Mbit ceil 6Mbit burst 15k cburst 15K prio 3
    tc filter add dev port_S4 parent 1: protocol ip prio 1 u32 match ip protocol 17 0xff match ip tos 32 0xff flowid 1:13

    # other
    tc class add dev port_S4 parent 1:1 classid 1:14 htb rate 450Kbit ceil 6Mbit burst 15k cburst 15K prio 4

    tc qdisc add dev port_S4 parent 1:11 handle 10: sfq perturb 10 limit 64 quantum 10000
    tc qdisc add dev port_S4 parent 1:12 handle 20: sfq perturb 10 limit 64 quantum 10000
    tc qdisc add dev port_S4 parent 1:13 handle 30: sfq perturb 10 limit 64 quantum 10000
    tc qdisc add dev port_S4 parent 1:14 handle 40: sfq perturb 10 limit 64 quantum 10000
...
tc class add dev port_S3 parent 1: classid 1:1 htb rate 4Mbit ceil 4Mbit burst 15k

    ## classes 

    # gold
    tc class add dev port_S3 parent 1:1 classid 1:11 htb rate 1Mbit ceil 4Mbit burst 15k cburst 15K prio 1 

    # silver
    tc class add dev port_S3 parent 1:1 classid 1:12 htb rate 4Mbit ceil 4Mbit burst 15k cburst 15K prio 2

    # bronze
    tc class add dev port_S3 parent 1:1 classid 1:13 htb rate 4Mbit ceil 4Mbit burst 15k cburst 15K prio 3

    # other
    tc class add dev port_S3 parent 1:1 classid 1:14 htb rate 450Kbit ceil 4Mbit burst 15k cburst 15K prio 4
